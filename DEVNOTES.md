...
Full debugging history and file list in `SESSION-HANDOVER.md`.

**Resolved — the "per-LOD boundary" limitation was chasing something that does not exist.** This was previously recorded here as still open, on the theory that the descriptor table lacked a per-LOD boundary marker and `BuildLodMeshGeometry` was therefore including/dropping stray vertices at LOD boundaries. There is no such boundary to find: these `.cga` files ship LOD1-5 as **separate sibling files**, so a section holds exactly one LOD. The `lod0*` naming inside that function is a historical misnomer. The real defect was the off-by-one descriptor table above, and the symptom that motivated the old theory (the "DR Model-XJ2 barrel_02" case in `SESSION-HANDOVER.md`) is explained by it. Do not re-open this as a reverse-engineering task.
