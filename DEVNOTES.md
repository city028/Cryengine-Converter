...
**Status**: `ChunkIvoLodMeshData`/`_900` now reads every `CAFEBABE` section in a file (previously only the first), mapping sections 1:1 to `Geometry`-type nodes in file order. Unbuildable sections (corrupt descriptors, bad index ranges) are skipped without invalidating the rest of the file. Two later fixes complete the picture:

- **Scale pivot** (`224903d`) — the `2.0` scale correction now pivots on the **decoded slice's own bounding-box centre**, not `QuantizationCenter`. `QuantizationCenter` is the centre of the quantization *volume*, not of the geometry, and the two differ whenever LOD0 vertices do not fill that volume symmetrically; because the scale step doubles the offset, such sections landed displaced by twice the difference (measured up to 0.086 on the BRVS chassis). Verified against `inventoryOccupancyLocalBounds` declared in each part's entity XML — data this converter plays no part in computing — at 0.9992–1.0008 across the BEHR SW16BR2 parts, against 1.260 for the barrel before the fix.
- **Final submesh** (`2683860`) — the descriptor table is offset by one: `entry[i].Packed` carries `(numVerts << 16 | numIndices)` for submesh *i+1*, not for entry *i*. The last entry therefore declares a final submesh with no cumulative row of its own, and reading counts from the last cumulative row alone silently discarded it. Emitting it recovered **+31,637 vertices (1.4%) across 52 downstream ship-weapon items**, concentrated in barrels; the worst single section was losing 55%. The fix is gated on the trailing submesh reconciling against the section header's own declared totals, so a corrupt table declines gracefully rather than reading past the buffer.

Full debugging history and file list in `SESSION-HANDOVER.md` (a local working document, not published to GitHub).

**Resolved — the "per-LOD boundary" limitation was chasing something that does not exist.** This was previously recorded here as still open, on the theory that the descriptor table lacked a per-LOD boundary marker and `BuildLodMeshGeometry` was therefore including/dropping stray vertices at LOD boundaries. There is no such boundary to find: these `.cga` files ship LOD1-5 as **separate sibling files**, so a section holds exactly one LOD. The `lod0*` naming inside that function is a historical misnomer. The real defect was the off-by-one descriptor table above, and the symptom that motivated the old theory (the "DR Model-XJ2 barrel_02" case, recorded in the local `SESSION-HANDOVER.md`) is explained by it. Do not re-open this as a reverse-engineering task.
