...
// Raw CryXmlB dump (.cdf and other binary-XML metadata files aren't recognized by the
// CryEngine model-loading pipeline at all - it only understands #ivo/CrCh/CryTek chunk
// signatures - so decode and print them directly, bypassing that pipeline entirely.
if (command == "--dump-cdf")
{
    var xmlDoc = CgfConverter.CryXmlB.CryXmlSerializer.ReadFile(filePath);
...
{
    // 2026-08-14 session 9: does each node's NodeMeshCombo bounding box describe where
    // that node's CAFEBABE section belongs in model space? (The skinned path already
    // rescales into these boxes; if lod sections match too, that is the missing link.)
    var model = cryData.Models[0];
    var combo = model.ChunkMap.Values.FirstOrDefault(c => c.ChunkType == ChunkType.NodeMeshCombo) as ChunkNodeMeshCombo;
    var lod = model.ChunkMap.Values.FirstOrDefault(c => c.ChunkType == ChunkType.IvoLodMeshData) as ChunkIvoLodMeshData;
    if (combo?.NodeMeshCombos is null || lod is null) { Console.WriteLine("missing chunk"); return; }

    int ord = 0;
    for (int i = 0; i < combo.NodeMeshCombos.Count; i++)
    {
        var n = combo.NodeMeshCombos[i];
        if (n.GeometryType != IvoGeometryType.Geometry) continue;
        string name = (combo.NodeNames is not null && i < combo.NodeNames.Count) ? combo.NodeNames[i] : $"node_{i}";
        var bmin = n.BoundingBoxMin; var bmax = n.BoundingBoxMax;
        var bc = (bmin + bmax) / 2f; var bs = bmax - bmin;
        Console.WriteLine($"node[{i}] {name}");
        Console.WriteLine($"   nodeBBox  center=({bc.X:F4},{bc.Y:F4},{bc.Z:F4}) size=({bs.X:F4},{bs.Y:F4},{bs.Z:F4})");
        if (ord < lod.Sections.Count)
        {
            var sec = lod.Sections[ord];
            var pos = sec.RawPositions;
            if (pos is not null && pos.Length > 0)
            {
                var mn = pos[0]; var mx = pos[0];
                foreach (var v in pos) { mn = System.Numerics.Vector3.Min(mn, v); mx = System.Numerics.Vector3.Max(mx, v); }
                var sc = (mn + mx) / 2f; var ss = mx - mn;
                Console.WriteLine($"   section[{ord}] verts={sec.VertexCount} center=({sc.X:F4},{sc.Y:F4},{sc.Z:F4}) size=({ss.X:F4},{ss.Y:F4},{ss.Z:F4})");
                Console.WriteLine($"   QuantCenter=({sec.QuantizationCenter.X:F4},{sec.QuantizationCenter.Y:F4},{sec.QuantizationCenter.Z:F4}) QuantScale=({sec.QuantizationScale.X:F4},{sec.QuantizationScale.Y:F4},{sec.QuantizationScale.Z:F4})");
            }
        }
        ord++;
    }
