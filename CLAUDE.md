...
**#ivo LOD Mesh Format (Star Citizen 4.5+)**: Static (non-skinned) geometry uses `ChunkIvoLodMeshData`/`ChunkIvoLodMeshData_900` instead of `ChunkIvoSkinMesh`. A file's geometry may span multiple `CAFEBABE`-tagged sections, each corresponding 1:1 to a `Geometry`-type node in file order — every section must be read and mapped to its own node, not just the first. Vertex positions are decoded relative to `QuantizationCenter` and require a `2.0` scale correction (`QuantizationScale` behaves as a half-extent). That scale must pivot on the **decoded slice's own bounding-box centre**, not on `QuantizationCenter` — the latter is the centre of the quantization volume rather than of the geometry, and pivoting there displaces the section by twice the difference. Note also that the descriptor table is offset by one (`entry[i].Packed` describes submesh *i+1*), so the final submesh has no cumulative row and must be emitted from the section header's declared totals. See `SESSION-HANDOVER.md` for the debugging history behind this.
